...
4. add your own Intel Developer SPID to `sgx-ra-tls/ra_tls_options.c`, e.g., if your SPID is 473BA3(...), add 0x47, 0x3B, 0xA3,(...)
...
6. add your own Intel Developer remote attestation subscription key to `sgx-ra-tls/ias-ra.c:228` by replacing the `"YYY"` with your subscription key

7. replace UID for container user, e.g., with `id --user` (cf. %TODO in dockerfile), in `base_container/Dockerfile`
...
7. run SENG server:
    ...
    ```
8. gracefully shutdown SENG server via ctrl+C (shutdown detection can take few seconds)
...
2. Install PSW version 2.7.1 at host, e.g., for Ubuntu 16.04 LTS:
    ```
    cd external/
...
## Notes for Running SENG
* ensure that you followed the setup steps above, s.t. all certificates and keys are in place, as well as the SENG interface and SGX driver/aesmd up and running.

* at the moment, you should only connect 1 enclave at once to the SENG server to avoid potential issues (cf. corresponding TODO entry)
...
## Running SENG Runtime Apps
*NOTE*: in rare cases the shutdown process of the SENG Runtime crashes with a 0x0 or SEGFAULT signal spam; we have not yet figured out why it happens sometimes; (might be fixed in newer Graphene versions where several race conditions/bugs got fixed)

### Demo App (SENG Runtime)
What it does:
...
* run `./test_seng_iperf.bash`

for Graphene-SGX w/o SENG (aka "pure"):
* run `./test_pure_iperf.bash`
...
for SENG:
* ensure SENG server is running
* run `./test_seng_curl.bash`

for pure:
* run `./test_pure_curl.bash`
...
for SENG:
* ensure SENG server is running
* run `./test_seng_telnet.bash <telnet_args>`

for pure:
* run `./test_pure_telnet.bash <telnet_args>`

for native:
* run `./test_native_telnet.bash <telnet_args>`
...
### NGINX (SENG Runtime, w/o auto-nat)
...
for pure:
* NGINX will be reachable under `192.168.178.45:4711/tcp` as specified in `conf/nginx_pure_and_native.conf`
* run `./test_pure_nginx.bash`

for SENG
* NGINX will be reachable via its *assigned Enclave IP* (cf. SENG server output) under `port 4711` (cf. `conf/nginx_seng.conf`)
* ensure SENG server is running
* run `./test_seng_nginx.bash`

Connecting from host to..
* ..pure NGINX via `netcat 192.168.178.45 4711`
* ..SENG NGINX via `netcat <enclave_ip> 4711` (e.g., 192.168.28.2)
...
Alternatively the (stripped) bench script `bench_with_wrk2.bash` can be used, or `wrk2` can be used directly, e.g.,
```
...
3. run NGINX:
    ...
    ```
4. connect to NGINX via two options:
    * via the *assigned Enclave IP* (cf. server output) on port 4711/tcp
    * via `192.168.178.45` on port 4711/tcp, for which DNAT forwarding rules have been auto-added
...
## Limitations, Future Features and more
