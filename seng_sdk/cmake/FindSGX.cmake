...
# Modified verison of https://github.com/zhang-xin/SGX-CMake/blob/master/cmake/FindSGX.cmake
...
message("SGX_TRTS_LIB: ${SGX_TRTS_LIB}")
message("SGX_URTS_LIB: ${SGX_URTS_LIB}")
...
if(SGX_FOUND)
    ...
    function(_build_edl_obj edl edl_search_paths use_prefix include_dirs)
        ...
        target_include_directories(${target}-edlobj PRIVATE ${CMAKE_CURRENT_BINARY_DIR} ${include_dirs})
    ...
    function(add_trusted_library target)
        ...
        set(oneValueArgs EDL LDSCRIPT OTHER_LIBS)
        set(multiValueArgs SRCS EDL_SEARCH_PATHS EDL_INCLUDE_DIRS)
        ...
        # : will only pass 1st list obj if no \""
        _build_edl_obj(${SGX_EDL} "${SGX_EDL_SEARCH_PATHS}" ${SGX_USE_PREFIX} ${SGX_EDL_INCLUDE_DIRS})
    ...
    function(add_enclave_library target)
        set(optionArgs USE_PREFIX)
        # : added "OTHER_LIBS" for specifying library/linker string to be linked BEFORE SGX SDK libs and with additional linker flags
        # --> was required for SGX SSL to link whole-archive correctly; otherwise custom CPUID exception handler was not called;
        set(oneValueArgs EDL LDSCRIPT OTHER_LIBS)
        # : ADD_CXX_FLAGS: to add own ones!
        # : EDL_INCLUDE_DIRS
        set(multiValueArgs SRCS TRUSTED_LIBS EDL_SEARCH_PATHS ADD_CXX_FLAGS EDL_INCLUDE_DIRS)
        cmake_parse_arguments("SGX" "${optionArgs}" "${oneValueArgs}" "${multiValueArgs}" ${ARGN})
        if("${SGX_EDL}" STREQUAL "")
            message(FATAL_ERROR "${target}: SGX enclave edl file is not provided!")
        endif()
        ...
        # : will only pass 1st list obj if no \""
        # : include dir for header files included inside .edl
        _build_edl_obj(${SGX_EDL} "${SGX_EDL_SEARCH_PATHS}" ${SGX_USE_PREFIX} ${SGX_EDL_INCLUDE_DIRS})

        add_library(${target} SHARED ${SGX_SRCS} $<TARGET_OBJECTS:${target}-edlobj>)

        # 
        set(SGX_ADD_CXX_FLAGS "${SGX_ADD_CXX_FLAGS} ${ENCLAVE_CXX_FLAGS}")
        set_target_properties(${target} PROPERTIES COMPILE_FLAGS ${SGX_ADD_CXX_FLAGS})

        target_include_directories(${target} PRIVATE ${CMAKE_CURRENT_BINARY_DIR})

        set(TLIB_LIST "")
        foreach(TLIB ${SGX_TRUSTED_LIBS})
            # : this doesn't work and spills errors, commented it out and made it easier for our case
            #string(APPEND TLIB_LIST "$<TARGET_FILE:${TLIB}> ")
            #add_dependencies(${target} ${TLIB})
            string(APPEND TLIB_LIST "${TLIB} ")
        endforeach()

        # : copied security hardening flags from SGX SSL sample app
        # : add switchless library at position described in sample code
        target_link_libraries(${target} "${SGX_COMMON_CFLAGS} \
            -Wl,-z,noexecstack -Wl,-z,relro -Wl,-z,now -pie"
            ${SGX_OTHER_LIBS}
            "-Wl,--no-undefined -nostdlib -nodefaultlibs -nostartfiles -L${SGX_LIBRARY_PATH} \
